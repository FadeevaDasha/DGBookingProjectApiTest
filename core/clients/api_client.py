...
class APIClient:
    ...
    def get_booking_by_id(self, Endpoints, booking_id, status_code=200):
        with allure.step('Getting a booking by ID'):
            url = f'{self.base_url}{Endpoints.BOOKING_ENDPOINT}{booking_id}'
            response = requests.get(url, headers=self.headers)

        with allure.step('Checking status code'):
            if status_code:
                assert response.status_code == status_code, f'Expected status 200 but got {response.status_code}'

        with allure.step('Checking the json response'):
            return response.json()